                ...
                if col not in  ['Tech', 'END_USES_DEMAND', 'CO2_DECENTRALISED', 'CO2_CENTRALISED','CO2_CAPTURED', 'CO2_ATMOSPHERE', 'URANIUM', 'OTHER_GHG', 'RES_WIND', 'RES_HYDRO', 'RES_SOLAR', 'RES_GEO']:
                    if i in list_storage:
                        ...
                        if i.startswith('TS_DEC'):
                            i = 'TS_DEC'
                        SOURCE, TARGET = col, i 
                        sankey_df = pd.concat([sankey_df, pd.DataFrame({'source': [SOURCE], 'target': [TARGET], 'realValue': [VAL_in]})], ignore_index=True)
                        sankey_df = pd.concat([sankey_df, pd.DataFrame({'source': [TARGET], 'target': [SOURCE], 'realValue': [VAL_out]})], ignore_index=True)
                    else:
                        VAL = dfi2[col].item()
                        if col == name_row:
                        ...
                        if abs(VAL)>500:
                            ...
                            sankey_df = pd.concat([sankey_df, pd.DataFrame({'source': [SOURCE], 'target': [TARGET], 'realValue': [VALUE]})], ignore_index=True)
...
def Sankey_carbon(df, gwp_construct):
    ...
    Returns:
        A Plotly Sankey plot.
    """
    resources = pd.read_csv(os.path.join(os.getcwd(), 'data', "resources.csv"), index_col=2, header=2, sep=';').reset_index()
        ...
        if i != 'END_USES_DEMAND': #Warning : There is a space before and after the name for the lines
            ...
            for col in dfi2:
                ...
                if col in  ['CO2_DECENTRALISED', 'CO2_CENTRALISED','CO2_CAPTURED', 'CO2_ATMOSPHERE', 'OTHER_GHG']:
                    ...
                    # Set a threshold to supress some small flows for a more efficient representation
                    if abs(VAL)>100:
                        if VAL >=0:
                            SOURCE, TARGET, VALUE = name_row, col, VAL
                        else:
    ...
    sankey_df = pd.concat([sankey_df, pd.DataFrame({'source': ['GHG_CONSTRUCTION'], 'target': ['CO2_ATMOSPHERE'], 'realValue': [gwp_construct['GWP_constr'].sum()]})], ignore_index=True)
...
def make_rgb_transparent(color, alpha):
    """Take a color in the form of text like 'red' and convert it into a rgb color and adding alpha as a value for transparency"""
