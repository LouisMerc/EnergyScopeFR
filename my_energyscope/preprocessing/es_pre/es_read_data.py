...
def import_data(config: dict):
    ...
    # Reading CSV
    # Assuming data_dir is the directory where your CSV files are located

    eud = pd.read_csv(data_dir / 'Demand.csv', index_col=2, header=0, sep=';')
    resources = pd.read_csv(data_dir / 'Resources.csv', index_col=2, header=2, sep=';')
    technologies = pd.read_csv(data_dir / 'Technologies.csv', index_col=3, header=0, skiprows=[1], sep=';')
    end_uses_categories = pd.read_csv(data_dir / 'END_USES_CATEGORIES.csv', sep='\t')
    layers_in_out = pd.read_csv(data_dir / 'Layers_in_out.csv', index_col=0, sep=';')
    storage_characteristics = pd.read_csv(data_dir / 'Storage_characteristics.csv', index_col=0, sep=';')
    storage_eff_in = pd.read_csv(data_dir / 'Storage_eff_in.csv', index_col=0, sep=';')
    storage_eff_out = pd.read_csv(data_dir / 'Storage_eff_out.csv', index_col=0, sep=';')
    time_series = pd.read_csv(data_dir / 'Time_series.csv', header=0, index_col=0, sep=';')
    # Reading misc.json
    ...
    storage_characteristics.dropna(axis=0, how='any', inplace=True)
